...
namespace CalyxEngine {
	...
	public:
		...
		static GeneratedShaderGraphCode GeneratePreviewPixelShaderSource(const MaterialAsset& material) {
			GeneratedShaderGraphCode materialFunction = GenerateObject3DMaterialFunction(material);
		...
		static GeneratedShaderGraphCode GenerateObject3DMaterialFunction(const CompiledMaterialGraph& graph) {
			...
			out << "GeneratedMaterialSurface EvaluateGeneratedMaterial(float2 uv) {\n";
	...
	private:
		...
		static GeneratedShaderGraphCode GeneratePreviewPixelShaderSource(const GeneratedShaderGraphCode& materialFunction) {
			...
			out << "    GeneratedMaterialSurface surface = EvaluateGeneratedMaterial(input.texcoord);\n";
...
PixelShaderOutput main(VertexShaderOutput input) {
    ...
    GeneratedMaterialSurface surface = EvaluateGeneratedMaterial(transformedUV.xy);
    ...
    return output;
		...
		static MaterialExpression ColorInputExpression(const MaterialAsset& material, const Node& node, const char* inputName, const Vector4& fallback) {
			...
			if(!pin || !HasInputLink(material.graph, pin->id)) return ColorExpression(fallback);
			return ColorExpressionFromInput(material.graph, pin->id, ColorExpression(fallback), 0);
		}
		...
		static MaterialExpression FloatInputExpression(const MaterialAsset& material, const Node& node, const char* inputName, float fallback) {
			const NodePin* pin = FindInput(node, inputName);
			if(!pin || !HasInputLink(material.graph, pin->id)) return FloatExpression(fallback);
			return FloatExpressionFromInput(material.graph, pin->id, FloatExpression(fallback), 0);
		}
		...
		static MaterialExpression ColorExpressionFromInput(
			...
			if(fromNode->type == "TextureSample") return {"gTexture.Sample(gSampler, uv)", true};
		...
		static MaterialExpressionSurface MakeDefaultExpressionSurface(const MaterialAsset& material) {
			MaterialExpressionSurface surface;
			surface.lightingMode = material.lightingMode;
			surface.baseColor = ColorExpression(material.color);
			surface.shininess = FloatExpression(material.shininess);
			surface.roughness = FloatExpression(material.roughness);
			surface.isReflect = material.isReflect;
			surface.toonHighlightColor = ColorExpression(material.toonHighlightColor);
			surface.toonBaseColor = ColorExpression(material.toonBaseColor);
			surface.toonFirstShadeColor = ColorExpression(material.toonMidShadowColor);
			surface.toonSecondShadeColor = ColorExpression(material.toonShadowColor);
			surface.toonBaseStep = FloatExpression(material.toonBaseStep);
			surface.toonBaseFeather = FloatExpression(material.toonBaseFeather);
			surface.toonShadeStep = FloatExpression(material.toonShadeStep);
			surface.toonShadeFeather = FloatExpression(material.toonShadeFeather);
			surface.toonSpecularThreshold = FloatExpression(material.toonSpecularThreshold);
			surface.toonSpecularSoftness = FloatExpression(material.toonSpecularSoftness);
			surface.toonSpecularIntensity = FloatExpression(material.toonSpecularIntensity);
			return surface;
		}
		...
		static MaterialExpressionSurface CompileMaterialExpressions(const MaterialAsset& material) {
			MaterialExpressionSurface surface = MakeDefaultExpressionSurface(material);
			...
			if(master->type == "ToonMaster") {
				...
				surface.baseColor = ColorInputExpression(material, *master, "Base Color", baseColor);
				surface.toonBaseColor = ColorExpression({1, 1, 1, 1});
				surface.toonHighlightColor = ColorInputExpression(material, *master, "Highlight", highlightColor);
				surface.toonFirstShadeColor = ColorInputExpression(material, *master, "1st Shade", firstShade);
				surface.toonSecondShadeColor = ColorInputExpression(material, *master, "2nd Shade", secondShade);
				surface.toonBaseStep = FloatInputExpression(material, *master, "Base Step", GetFloatProperty(*master, "baseStep", 0.25f));
				surface.toonBaseFeather = FloatInputExpression(material, *master, "Base Feather", GetFloatProperty(*master, "baseFeather", 0.03f));
				surface.toonShadeStep = FloatInputExpression(material, *master, "Shade Step", GetFloatProperty(*master, "shadeStep", -0.15f));
				surface.toonShadeFeather = FloatInputExpression(material, *master, "Shade Feather", GetFloatProperty(*master, "shadeFeather", 0.03f));
				surface.toonSpecularThreshold = FloatInputExpression(material, *master, "Spec Threshold", GetFloatProperty(*master, "specularThreshold", 0.96f));
				surface.toonSpecularSoftness = FloatInputExpression(material, *master, "Spec Softness", GetFloatProperty(*master, "specularSoftness", 0.02f));
				surface.toonSpecularIntensity = FloatInputExpression(material, *master, "Spec Intensity", GetFloatProperty(*master, "specularIntensity", 0.35f));
				return surface;
			}

			if(master->type == "LitMaster") {
				surface.lightingMode = static_cast<int32_t>(GetFloatProperty(*master, "lightingMode", 0.0f));
				surface.baseColor = ColorInputExpression(material, *master, "Base Color", material.color);
				surface.shininess = FloatInputExpression(material, *master, "Shininess", GetFloatProperty(*master, "shininess", material.shininess));
				surface.roughness = FloatInputExpression(material, *master, "Roughness", GetFloatProperty(*master, "roughness", material.roughness));
				return surface;
			}
